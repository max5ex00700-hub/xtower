...
public sealed class XTapBuildConfig : IPreprocessBuildWithReport
{
    ...
    static void ValidateReferenceUiAssets()
    {
        ...
        for (int i = 0; i < required.Length; i++)
        {
            ...
            if (!File.Exists(path))
                throw new BuildFailedException("X탑 11.18 필수 메인 UI 자산 누락: " + required[i]);
            ...
            {
                throw new BuildFailedException("X탑 11.18 메인 UI 자산 손상: " + required[i] + " / " + e.Message);
            }
        }

        Debug.Log("X탑 11.18 메인 UI 검증 완료: 기준 이미지 버튼 9개 정상.");
    }
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        PlayerSettings.bundleVersion = "11.18-next-button-fix-" + shortCommit;
        ...
        PlayerSettings.Android.bundleVersionCode = 1118;
