...
public sealed class XTapBuildConfig : IPreprocessBuildWithReport
{
    ...
    static void ValidateReferenceUiAssets()
    {
        ...
        for (int i = 0; i < required.Length; i++)
        {
            ...
            if (!File.Exists(path))
                throw new BuildFailedException("X탑 11.39 필수 메인 UI 자산 누락: " + required[i]);
            ...
            {
                throw new BuildFailedException("X탑 11.39 메인 UI 자산 손상: " + required[i] + " / " + e.Message);
            }
        }
        ...
        if (!File.Exists(forgeWheelPath))
            throw new BuildFailedException("X탑 11.39 대장간 SUCCESS/FAIL 룰렛 에셋 누락.");
        ...
        if (forgeWheel.Length < 1024 ||
            ...
            throw new BuildFailedException("X탑 11.39 대장간 SUCCESS/FAIL 룰렛 에셋 손상.");
        ...
        for (int i = 0; i < gachaAssets.Length; i++)
        {
            ...
            if (!File.Exists(path))
                throw new BuildFailedException("X탑 11.39 블록 머신 에셋 누락: " + gachaAssets[i]);

            byte[] image = File.ReadAllBytes(path);
            if (image.Length < 1024 || image[0] != 0xFF || image[1] != 0xD8 || image[2] != 0xFF)
                throw new BuildFailedException("X탑 11.39 블록 머신 에셋 손상: " + gachaAssets[i]);
        }

        Debug.Log("X탑 11.39 UI 검증 완료: 메인 버튼 + 대장간 룰렛 + 9:16 블록 머신 에셋 정상.");
    }
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        PlayerSettings.bundleVersion = "11.39-full-art-wheel-mask-" + shortCommit;
        ...
        PlayerSettings.Android.bundleVersionCode = 1139;
